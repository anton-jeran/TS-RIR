...
def TSRIRGANGenerator_real(
  ...
  # Layer 4
  # [64, 512] -> [16, 1024] %[32,1024]
  with tf.variable_scope('downconv_4'):
    output = tf.layers.conv1d(output, dim * 8, kernel_len, 1, padding='SAME')
    output = batchnorm(output)
  output = tf.nn.relu(output)
  ...
  with tf.variable_scope('downconv_7'):
    ...
    output = batchnorm(output)
  output = tf.nn.relu(output)    
  with tf.variable_scope('downconv_8'):
    output = tf.layers.conv1d(output, dim * 8, kernel_len, 1, padding='SAME')
    output = batchnorm(output)
  output = tf.nn.relu(output)  

  # # Layer 0
  # # [16, 1024] -> [64, 512] %[32, 1024]
  # with tf.variable_scope('upconv_0'):
  #   output = conv1d_transpose(output, dim * dim_mul, kernel_len, 2, upsample=upsample)
  #   output = batchnorm(output)
  # output = tf.nn.relu(output)
  # dim_mul //= 2
...
def TSRIRGANGenerator_synthetic(
    z,
    slice_len=16384,
    nch=1,
    kernel_len=25,
...
"""
def TSRIRGANDiscriminator_synthetic(
    x,
    kernel_len=25,
    dim=64,
    use_batchnorm=False,
...
def TSRIRGANDiscriminator_real(
    x,
    kernel_len=25,
    dim=64,
    use_batchnorm=False,
